...
class InmuebleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($params_array)) {
            ...
            if ($validate->fails()) {
                ...
                $id = IdGenerator::generate(['table' => 't_unidades','field'=>'Proyecto_ID', 'length' => 10,'prefix' =>date('ymd')]);
                ...
                $dt->id_unidad                  = $id;
                $dt->Torre_Name                 = $params_array['Torre_Name'];
    ...
    function update($id, Request $request)
    {
        ...
        if (!empty($params_array)) {
            ...
            unset($params_array['id_unidad']);
            unset($params_array['User_ID']);
            unset($params_array['created_at']);

            $obra = Inmueble::where('id_unidad', $id)->update($params_array);
            $data = [
    ...
    public function filtroInmueblesTorre($torre,$proyecto)
    {
        $data = DB::select(

            'SELECT *  FROM t_unidades WHERE  	Torre_Name=? AND Proyecto_ID=?',
            [$torre,$proyecto]
